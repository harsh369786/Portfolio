    ...
    <section id="services" className="relative z-20 bg-background px-6 py-32 md:px-24 border-t border-white/5">
      <div className="max-w-6xl mx-auto">
        <div className="mb-20 text-center md:text-left">
          ...
          <h2 className="text-4xl md:text-6xl font-heading font-bold mb-4 tracking-tight text-white">Services Built for Founders & Businesses</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-32">
          {SERVICES.map((service, index) => (
            <motion.div
              ...
              className="group bg-[#111111] p-10 rounded-3xl border border-white/5 transition-all duration-300 hover:border-accent hover:shadow-[0_0_30px_rgba(59,130,246,0.1)]"
            >
              <div className="text-accent mb-6 transition-transform group-hover:scale-110 duration-300">
                {service.icon}
              </div>
              <h3 className="text-xl md:text-2xl font-bold mb-4 group-hover:text-accent transition-colors text-white">
                {service.title}
              </h3>
        ...
        <div>
          <h3 className="text-2xl font-heading font-bold mb-16 text-center text-white">How I Work</h3>
          <div className="relative flex flex-col md:flex-row justify-between gap-12 md:gap-4 px-4">
            ...
            {PROCESS.map((p, index) => (
              ...
              >
                <div className="w-12 h-12 rounded-full bg-[#111111] border-2 border-accent flex items-center justify-center font-bold text-accent mb-6 shadow-[0_0_15px_rgba(59,130,246,0.3)]">
                  {p.step}
                </div>
                <h4 className="text-base font-bold mb-2 text-white">{p.name}</h4>
